  ...
  <div class="board_block">
    ...
    <div class="width-ful">
      <div class="layout">
          ...
          <div class="layout__list">
            <div class="layout__list__title">
              ...
              <button
                id=""
                class="ft-r mrgn-2"
                @click="removeColumn(listItem.id)"
              >
                X
              </button>
            </div>
            <div
              class="text-a-center taskAdd"
              v-on:click="handleAddTaskClicked(listItem)"
            >
              <span> +Add Task</span>
            </div>
            <draggable
              ...
              @end="handleTaskMoved"
            >
              <div v-for="(listItem, index) in listItem.tasks" :key="index">
                <div
                  class="list__card"
                  v-on:click="handleTaskClicked(listItem)"
                >
        ...
        <div class="layout__list">
          <div
            class="layout__list__title pad-y-5"
            v-on:click="handleAddColClicked()"
          >
...
export default {
  ...
  methods: {
    ...
    editTaskClicked() {
      ...
      axios
        .put(`/tasks/${this.editTask.id}`, payload)
        .then((response) => {
    ...
    addTaskClicked() {
      let newTask = {
        ...
        column_id: this.addTask.column_id,
      };
    ...
    addColumnBtnClicked() {
      ...
      axios.post("/columns", newColumn).then((response) => {
        this.initial_data = response.data;
      });
    ...
    removeColumn(colId) {
      let data = { id: colId };
      axios.delete(`/columns/${colId}`).then((response) => {
        const columnIndex = this.initial_data.findIndex(
